...
// =============================================================================
// SENSORS & NATIVE CALLS - Generic and one-line access
// =============================================================================

/**
 * Generic native call - call any DroidScript sensor or native API.
 * 
 * All logic runs in JS (DroidScript). PHP only receives the result via callback.
 * Built-in types: location, battery, accelerometer, compass, gyroscope, gravity,
 *   magneticfield, proximity, light, pressure, humidity, temperature, stepcounter,
 *   deviceinfo, screeninfo, locationenabled, wifi, networkinfo, wifiscan,
 *   bluetooth, btdiscover, nfc, http, download, camera, playaudio, stopaudio,
 *   recordaudio, stoprecording, ringtone, getvolume, setvolume, setringermode,
 *   setbrightness, preventscreenlock, vibrate, flashlight, speech,
 *   speechrecognition, sms, phonecall, sendemail, notification,
 *   clipboard_set, clipboard_get, encrypt, decrypt, hash,
 *   readfile, writefile, listfolder, fileexists, openapp, intent
 * 
 * You can also register custom handlers in JS with:
 *   php.RegisterNativeHandler("myType", function(params) { ... });
 * 
 * @param string $type The native call type (sensor name or custom handler name)
 * @param array $params Parameters to pass to the handler (optional)
 * @param string $callback PHP method to receive the result (optional)
 * @return array
 * 
 * Example:
 *   // Request GPS location
 *   function getLocation($p) { return nativeCall("location", [], "onGps"); }
 *   function onGps($p) { return setText("lbl", "Lat: " . $p['lat']); }
 * 
 *   // HTTP GET request
 *   function fetchData($p) { 
 *     return nativeCall("http", ["url" => "https://api.example.com/data", "httpMethod" => "GET"], "onData"); 
 *   }
 * 
