...
{
    section: "Native API (Pure Java)", icon: "⚡", desc: "75 native handlers: battery, HTTP, camera, files, zip, crypto, sensors, intents — all pure Java, no DroidScript.",
    items: [
        { name: "native", sig: 'native(string $type, string $callback, array $params = [])', desc: "Call a native Java handler directly. This is the core function for 75+ native operations — sensors, files, HTTP, camera, crypto, zip, intents, and more. Executes in pure Java without DroidScript bridge.",
          params: [["$type","Handler type (e.g., 'battery', 'http', 'zipfolder')"],["$callback","Method to receive the result"],["$params","Optional parameters for the handler"]],
          example: '<?php\nrequire_once dirname(__FILE__)."/"."simple.php";\n\nclass MyApp {\n    function index() {\n        return page("Native API", [\n            label("---", ["id" => "result"]),\n            button("Get Battery", "doBattery"),\n            button("HTTP GET", "doHttp"),\n        ]);\n    }\n    function doBattery($p) {\n        return native("battery", "onBattery");\n    }\n    function onBattery($p) {\n        $level = $p["level"] ?? 0;\n        $charging = ($p["charging"] ?? false) ? "Yes" : "No";\n        return setText("result", "Battery: $level% | Charging: $charging");\n    }\n    function doHttp($p) {\n        return native("http", "onHttp", ["url" => "https://httpbin.org/get"]);\n    }\n    function onHttp($p) {\n        $resp = $p["response"] ?? $p["error"] ?? "No data";\n        return setText("result", substr($resp, 0, 200));\n    }\n}' },
        { name: "Native Handlers — Sensors", sig: 'native("battery", $cb)\nnative("accelerometer", $cb)\nnative("gyroscope", $cb)\nnative("compass", $cb)\nnative("light", $cb)\nnative("proximity", $cb)\nnative("pressure", $cb)\nnative("humidity", $cb)\nnative("temperature", $cb)\nnative("stepcounter", $cb)\nnative("magneticfield", $cb)\nnative("gravity", $cb)\nnative("orientation", $cb)', desc: "Sensor handlers return sensor data as JSON. Battery returns {level, charging}. Motion sensors return {x, y, z, values}. Environment sensors return {value}.",
          params: [["$cb","Callback receives sensor data JSON"]],
          example: '<?php\nrequire_once dirname(__FILE__)."/"."simple.php";\n\nclass MyApp {\n    function index() {\n        return page("Sensors", [\n            label("---", ["id" => "data"]),\n            button("Accelerometer", "doAccel"),\n            button("Light", "doLight"),\n            button("Compass", "doCompass"),\n        ]);\n    }\n    function doAccel($p) { return native("accelerometer", "onSensor"); }\n    function doLight($p) { return native("light", "onSensor"); }\n    function doCompass($p) { return native("compass", "onSensor"); }\n    function onSensor($p) {\n        return setText("data", json_encode($p));\n    }\n}' },
        { name: "Native Handlers — Location", sig: 'native("gps", $cb)\nnative("lastknownlocation", $cb)\nnative("address", $cb, ["lat"=>$lat,"lng"=>$lng])\nnative("geocode", $cb, ["address"=>$addr])\nnative("locationenabled", $cb)', desc: "Location handlers. GPS returns {lat, lng, altitude, accuracy, speed, bearing}. Address/geocode convert between coordinates and addresses.",
          params: [["$cb","Callback receives location data"],["lat/lng","Coordinates for reverse geocoding"],["address","Address string for geocoding"]],
          example: '<?php\nrequire_once dirname(__FILE__)."/"."simple.php";\n\nclass MyApp {\n    function index() {\n        return page("Location", [\n            label("---", ["id" => "loc"]),\n            button("Get GPS", "doGps"),\n            button("Geocode NYC", "doGeocode"),\n        ]);\n    }\n    function doGps($p) { return native("gps", "onGps"); }\n    function onGps($p) {\n        $lat = $p["lat"] ?? "?";\n        $lng = $p["lng"] ?? "?";\n        return setText("loc", "Lat: $lat, Lng: $lng");\n    }\n    function doGeocode($p) {\n        return native("geocode", "onGeo", ["address" => "New York, NY"]);\n    }\n    function onGeo($p) {\n        return setText("loc", "Lat: ".($p["lat"]??"").", Lng: ".($p["lng"]??""));\n    }\n}' },
        { name: "Native Handlers — HTTP & Network", sig: 'native("http", $cb, ["url"=>$url])\nnative("http", $cb, ["url"=>$url,"method"=>"POST","body"=>$json,"contentType"=>"application/json"])\nnative("download", $cb, ["url"=>$url,"dest"=>$path])\nnative("networkinfo", $cb)\nnative("wifiinfo", $cb)\nnative("wifiscan", $cb)\nnative("bluetoothinfo", $cb)\nnative("ping", $cb, ["host"=>$host])', desc: "HTTP requests, downloads, and network info. HTTP supports GET/POST with headers. Download saves files. Network handlers return connectivity status.",
          params: [["url","Request URL"],["method","GET, POST, PUT, DELETE"],["body","Request body for POST"],["contentType","MIME type"],["dest","Download destination path"]],
          example: '<?php\nrequire_once dirname(__FILE__)."/"."simple.php";\n\nclass MyApp {\n    function index() {\n        return page("Network", [\n            label("---", ["id" => "net"]),\n            button("GET Request", "doGet"),\n            button("POST JSON", "doPost"),\n            button("Download", "doDownload"),\n        ]);\n    }\n    function doGet($p) {\n        return native("http", "onHttp", ["url" => "https://httpbin.org/get"]);\n    }\n    function doPost($p) {\n        return native("http", "onHttp", [\n            "url" => "https://httpbin.org/post",\n            "method" => "POST",\n            "body" => json_encode(["name" => "PHP"]),\n            "contentType" => "application/json"\n        ]);\n    }\n    function doDownload($p) {\n        return native("download", "onDownload", [\n            "url" => "https://picsum.photos/200",\n            "dest" => "/sdcard/Download/photo.jpg"\n        ]);\n    }\n    function onHttp($p) { return setText("net", substr($p["response"] ?? $p["error"] ?? "", 0, 200)); }\n    function onDownload($p) { return toast(($p["success"] ?? false) ? "Downloaded!" : "Failed"); }\n}' },
        { name: "Native Handlers — Files & Zip", sig: 'native("readfile", $cb, ["path"=>$p])\nnative("writefile", $cb, ["path"=>$p,"content"=>$c])\nnative("listfiles", $cb, ["path"=>$p])\nnative("fileexists", $cb, ["path"=>$p])\nnative("deletefile", $cb, ["path"=>$p])\nnative("mkdir", $cb, ["path"=>$p])\nnative("copyfile", $cb, ["src"=>$s,"dest"=>$d])\nnative("movefile", $cb, ["src"=>$s,"dest"=>$d])\nnative("fileinfo", $cb, ["path"=>$p])\nnative("zipfolder", $cb, ["folder"=>$f,"zip"=>$z])\nnative("unzip", $cb, ["zip"=>$z,"dest"=>$d])', desc: "File operations and zip handling. All file handlers work with device storage. Zip handlers create/extract archives.",
          params: [["path","File or folder path"],["content","Content to write"],["src/dest","Source and destination paths"],["folder/zip","Folder to zip and output zip path"]],
          example: '<?php\nrequire_once dirname(__FILE__)."/"."simple.php";\n\nclass MyApp {\n    function index() {\n        return page("Files", [\n            label("---", ["id" => "files"]),\n            button("Write File", "doWrite"),\n            button("Read File", "doRead"),\n            button("Zip Folder", "doZip"),\n        ]);\n    }\n    function doWrite($p) {\n        return native("writefile", "onWrite", [\n            "path" => "/sdcard/test.txt",\n            "content" => "Hello from PHP native()!"\n        ]);\n    }\n    function onWrite($p) { return toast(($p["success"] ?? false) ? "Written!" : "Failed"); }\n    function doRead($p) {\n        return native("readfile", "onRead", ["path" => "/sdcard/test.txt"]);\n    }\n    function onRead($p) { return setText("files", $p["content"] ?? $p["error"] ?? ""); }\n    function doZip($p) {\n        return native("zipfolder", "onZip", [\n            "folder" => "/sdcard/MyFolder",\n            "zip" => "/sdcard/MyFolder.zip"\n        ]);\n    }\n    function onZip($p) { return toast(($p["success"] ?? false) ? "Zipped!" : "Failed"); }\n}' },
        { name: "Native Handlers — Crypto", sig: 'native("encrypt", $cb, ["text"=>$t,"password"=>$p])\nnative("decrypt", $cb, ["text"=>$t,"password"=>$p])\nnative("md5", $cb, ["text"=>$t])\nnative("sha256", $cb, ["text"=>$t])\nnative("sha1", $cb, ["text"=>$t])\nnative("sha512", $cb, ["text"=>$t])\nnative("base64encode", $cb, ["text"=>$t])\nnative("base64decode", $cb, ["text"=>$t])\nnative("uuid", $cb)\nnative("randomstring", $cb, ["length"=>$n])', desc: "Encryption, hashing, and encoding. AES encryption with password. Multiple hash algorithms. Base64 encoding/decoding. UUID and random string generation.",
          params: [["text","Input text"],["password","Encryption key"],["length","Random string length"]],
          example: '<?php\nrequire_once dirname(__FILE__)."/"."simple.php";\n\nclass MyApp {\n    function index() {\n        return page("Crypto", [\n            input("txt", "Text to process"),\n            label("---", ["id" => "result"]),\n            button("Encrypt", "doEncrypt"),\n            button("MD5", "doMd5"),\n            button("UUID", "doUuid"),\n        ]);\n    }\n    function doEncrypt($p) { return getText("txt", "encryptIt"); }\n    function encryptIt($p) {\n        return native("encrypt", "onResult", [\n            "text" => $p["value"],\n            "password" => "secret123"\n        ]);\n    }\n    function doMd5($p) { return getText("txt", "hashIt"); }\n    function hashIt($p) {\n        return native("md5", "onResult", ["text" => $p["value"]]);\n    }\n    function doUuid($p) { return native("uuid", "onResult"); }\n    function onResult($p) { return setText("result", $p["result"] ?? ""); }\n}' },
        { name: "Native Handlers — Media & Camera", sig: 'native("takephoto", $cb)\nnative("takevideo", $cb)\nnative("pickimage", $cb)\nnative("pickvideo", $cb)\nnative("pickfile", $cb)\nnative("playsound", $cb, ["file"=>$f])\nnative("stopsound", $cb)\nnative("recordaudio", $cb, ["file"=>$f])\nnative("stoprecording", $cb)\nnative("speak", $cb, ["text"=>$t])\nnative("speechrecognition", $cb)', desc: "Camera, media picker, audio playback/recording, and speech. Camera handlers return {uri}. Audio handlers control playback/recording.",
          params: [["file","Audio file path"],["text","Text for speech synthesis"]],
          example: '<?php\nrequire_once dirname(__FILE__)."/"."simple.php";\n\nclass MyApp {\n    function index() {\n        return page("Media", [\n            label("---", ["id" => "media"]),\n            button("Take Photo", "doPhoto"),\n            button("Pick Image", "doPick"),\n            button("Speak", "doSpeak"),\n        ]);\n    }\n    function doPhoto($p) { return native("takephoto", "onMedia"); }\n    function doPick($p) { return native("pickimage", "onMedia"); }\n    function onMedia($p) {\n        return setText("media", "URI: " . ($p["uri"] ?? "none"));\n    }\n    function doSpeak($p) {\n        return native("speak", "_noop", ["text" => "Hello from PHP!"]);\n    }\n}' },
        { name: "Native Handlers — Device & System", sig: 'native("deviceinfo", $cb)\nnative("screeninfo", $cb)\nnative("packageinfo", $cb)\nnative("installedapps", $cb)\nnative("vibrate", $cb, ["duration"=>$ms])\nnative("flashlight", $cb, ["on"=>true])\nnative("brightness", $cb, ["level"=>0.5])\nnative("volume", $cb)\nnative("setvolume", $cb, ["level"=>$lvl,"stream"=>$s])\nnative("clipboard", $cb)\nnative("setclipboard", $cb, ["text"=>$t])\nnative("openurl", $cb, ["url"=>$u])\nnative("sharetext", $cb, ["text"=>$t])', desc: "Device info, hardware control, clipboard, and sharing. Device/screen info return specifications. Hardware handlers control flashlight, vibration, brightness, volume.",
          params: [["duration","Vibration duration in ms"],["on","Flashlight on/off"],["level","Brightness/volume level"],["stream","audio stream: music, ring, alarm"],["text","Text to copy/share"],["url","URL to open"]],
          example: '<?php\nrequire_once dirname(__FILE__)."/"."simple.php";\n\nclass MyApp {\n    function index() {\n        return page("System", [\n            label("---", ["id" => "info"]),\n            button("Device Info", "doDevice"),\n            button("Flashlight", "doFlash"),\n            button("Vibrate", "doVibrate"),\n        ]);\n    }\n    function doDevice($p) { return native("deviceinfo", "onInfo"); }\n    function onInfo($p) {\n        return setText("info", json_encode($p));\n    }\n    function doFlash($p) {\n        return native("flashlight", "_noop", ["on" => true]);\n    }\n    function doVibrate($p) {\n        return native("vibrate", "_noop", ["duration" => 500]);\n    }\n}' },
        { name: "Native Handlers — SMS & Intents", sig: 'native("sendsms", $cb, ["phone"=>$p,"message"=>$m])\nnative("contacts", $cb)\nnative("calllogs", $cb, ["limit"=>50])\nnative("launchapp", $cb, ["package"=>$pkg])\nnative("sendintent", $cb, ["action"=>$a,"uri"=>$u,"type"=>$t,"extras"=>$e])\nnative("scanbarcode", $cb)', desc: "SMS, contacts, call logs, app launching, and intents. SMS sends messages directly. Intent handler launches activities with custom actions.",
          params: [["phone","Phone number"],["message","SMS text"],["package","App package name"],["action","Intent action"],["uri","Intent data URI"],["type","MIME type"],["extras","JSON extras object"]],
          example: '<?php\nrequire_once dirname(__FILE__)."/"."simple.php";\n\nclass MyApp {\n    function index() {\n        return page("Communication", [\n            label("---", ["id" => "comm"]),\n            button("Send SMS", "doSms"),\n            button("Launch App", "doApp"),\n            button("Open URL", "doUrl"),\n        ]);\n    }\n    function doSms($p) {\n        return native("sendsms", "onSms", [\n            "phone" => "+1234567890",\n            "message" => "Hello from PHP!"\n        ]);\n    }\n    function onSms($p) { return toast(($p["success"] ?? false) ? "Sent!" : "Failed"); }\n    function doApp($p) {\n        return native("launchapp", "_noop", ["package" => "com.google.android.gm"]);\n    }\n    function doUrl($p) {\n        return native("sendintent", "_noop", [\n            "action" => "android.intent.action.VIEW",\n            "uri" => "https://php.net"\n        ]);\n    }\n}' },
        { name: "All 75 Native Handlers", sig: 'Sensors: battery, accelerometer, gyroscope, compass, light, proximity, pressure, humidity, temperature, stepcounter, magneticfield, gravity, orientation\nLocation: gps, lastknownlocation, address, geocode, locationenabled\nNetwork: http, download, networkinfo, wifiinfo, wifiscan, bluetoothinfo, ping\nFiles: readfile, writefile, appendfile, listfiles, fileexists, deletefile, mkdir, copyfile, movefile, fileinfo\nZip: zipfolder, zipfiles, unzip, listzip\nCrypto: encrypt, decrypt, md5, sha1, sha256, sha512, base64encode, base64decode, uuid, randomstring\nMedia: takephoto, takevideo, pickimage, pickvideo, pickfile, playsound, stopsound, recordaudio, stoprecording, speak, speechrecognition\nDevice: deviceinfo, screeninfo, packageinfo, installedapps, vibrate, flashlight, brightness, volume, setvolume, ringermode, keepscreenon\nClipboard: clipboard, setclipboard, openurl, sharetext\nCommunication: sendsms, contacts, calllogs, launchapp, sendintent, scanbarcode, notify', desc: "Complete list of all 75 native handlers available through the native() function. All execute in pure Java without the DroidScript JavaScript bridge.",
          params: [],
          example: '// Quick Reference:\n// native("battery", "onResult");\n// native("http", "onResult", ["url" => "..."]);\n// native("zipfolder", "onResult", ["folder" => "...", "zip" => "..."]);\n// native("encrypt", "onResult", ["text" => "...", "password" => "..."]);\n// native("takephoto", "onResult");' },
    ]
},
{
    section: "Sensors (Legacy)", icon: "📡", desc: "GPS, accelerometer, compass via convenience wrappers (use native() for pure Java).",
    items: [
...
function GetSystemPrompt() {
    ...
    prompt += "8) Always use require_once 'simple.php' \n\n";
